  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="lg:col-span-8 space-y-8">
        ...
        {/* Header Hero Card */}
        <div className="relative bg-sky-50 border border-sky-100 rounded-3xl p-8 overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-sky-100/50 rounded-full -mr-20 -mt-20 blur-3xl"></div>
          <div className="relative z-10">
            <h1 className="text-3xl font-bold text-slate-900 font-outfit mb-4">
              Hi {user?.name.split(' ')[0]} <span className="animate-bounce inline-block">👋</span>
            </h1>
            <div className="flex flex-wrap gap-4 text-sm text-slate-600">
              <div className="flex items-center bg-white/60 px-3 py-1.5 rounded-full border border-sky-200/50">
          ...
          {/* Logo element on the right */}
          <div className="absolute right-8 top-1/2 -translate-y-1/2 hidden md:block">
            <div className="w-24 h-24 bg-white rounded-2xl shadow-xl flex items-center justify-center border border-slate-100 p-2 transform rotate-6">
      ...
      <div className="lg:col-span-4 space-y-8">
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { name: 'Livebooks', icon: BookOpen, color: 'text-purple-500' },
              { name: 'Dojo', icon: Trophy, color: 'text-red-500' },
              { name: 'Mail', icon: Megaphone, color: 'text-blue-500' },
              { name: 'Chat', icon: MessageSquare, color: 'text-orange-500' },
              { name: 'Calendar', icon: Calendar, color: 'text-emerald-500' },
              { name: 'Support', icon: Users, color: 'text-rose-500' },
            ].map(app => (
              <button key={app.name} className="flex flex-col items-center justify-center p-3 rounded-2xl border border-slate-100 hover:border-kalvium/30 hover:bg-slate-50 transition-all group">
                <div className={`p-2 rounded-xl bg-slate-50 group-hover:bg-white mb-2 ${app.color}`}>
        ...
        <div className="bg-kalvium/5 border border-kalvium/10 rounded-3xl p-6">
          ...
          <button className="w-full py-2.5 bg-kalvium text-white font-bold rounded-xl shadow-lg shadow-kalvium/20 hover:scale-[1.02] active:scale-95 transition-all">
            Get Started
          </button>
        </div>
      </div>
